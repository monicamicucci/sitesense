...
class FilteringRankingService:
    ...
    def _format_to_html(self, activities: List[Dict[str, Any]], warning: Optional[str]) -> str:
        ...
        for activity in activities:
            ...
            pid = str(activity.get('place_id',''))
            if pid and not pid.startswith('gemini-'):
                maps_url = f"https://www.google.com/maps/place/?q=place_id:{pid}"
            else:
                from urllib.parse import quote_plus
                name_addr = (name + ' ' + (activity.get('formatted_address') or activity.get('vicinity') or 'Italia')).strip()
            ...
            # Ottieni l'URL dell'immagine se disponibile
            photo_url = self._get_photo_url(activity.get('photos'))
            ...
            links_html = f'<div class="links" style="margin-top: 0px;"> <a href="{maps_url}" target="_blank" class="selection-card-link" onclick="openActivityModal(\'{activity['place_id']}\', \'{name.replace("'", "&#39;")}\'); return false;" style="display: inline-flex; align-items: center; gap: 6px; text-decoration: none;font-size: 0.875rem; font-weight: 600; color: #0f3e34;">Visualizza la scheda<span class="material-icons" style="font-size: 16px; line-height: 1; color: #0f3e34;">arrow_forward</span></a></div>'
...
<h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 2px; line-height: 1.25; color: #0f172a;">
    ...
    def _create_our_selection(self, all_activities: Dict[str, List[Dict[str, Any]]]) -> Optional[str]:
        ...
        la migliore attività solo per le categorie specifiche: Hotel, Vini, Cucina tipica.
        """
        selected_items = []
        ...
        target_categories = {
            ...
            'cucina_tipica': {
                'keywords': ['cucina_tipica', 'ristoranti', 'restaurant', 'food', 'dining', 'trattoria'],
                'limit': 2
            }
        }
            ...
            for category, activities in all_activities.items():
                ...
                if activities and any(keyword in category.lower() for keyword in keywords):
                    for act in activities:
                        ...
                        count_added += 1
                        logger.info(f"Selezionata attività '{item.get('name')}' per la categoria target '{target_key}' dalla categoria '{category}'")
        ...
        try:
            seen_ids = set([it.get('place_id') for it in selected_items if it.get('place_id')])
                ...
                for cat, activities in all_activities.items():
                    for act in activities:
                        ...
                        if pid and pid not in seen_ids:
                            extra = dict(act)
                            extra['display_category'] = extra.get('display_category') or cat
        ...
        <div class="our-selection" style="margin: 16px 16px 16px 48px; padding: 28px 28px 32px 32px; background-color: #0f3e34; border-radius: 1px; color: #ffffff; max-width: 1400px;">
            ...
            <p style="margin: 0 0 16px 0; font-size: 0.8rem; text-align: left; color: #c7d2d9;">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrum exercitationem.</p>
        ...
        for item in selected_items:
            ...
            pid2 = str(item.get('place_id',''))
            if pid2 and not pid2.startswith('gemini-'):
                maps_url = f"https://www.google.com/maps/place/?q=place_id:{pid2}"
            else:
                from urllib.parse import quote_plus
                name_addr2 = (name + ' ' + (item.get('formatted_address') or item.get('vicinity') or 'Italia')).strip()
                maps_url = f"https://www.google.com/maps/search/?api=1&query={quote_plus(name_addr2)}"
            ...
            # Genera l'HTML per l'immagine (sempre CSE)
            image_html = f'''<div class="activity-image" style="width: 250px; height: 200px; background-image: url({photo_url}); background-size: cover; background-position: center; border-radius: 8px 8px 0 0; margin: -16px -16px 12px -16px;"></div>'''
  ...
  onmouseleave="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 8px rgba(0,0,0,0.1)'; try{{this.querySelector('.minus-btn').style.opacity='0';}}catch(e){{}}">
                ...
                <button type="button" class="minus-btn" title="Rimuovi"
                    ...
                    >-</button>
                <!-- Contenuto della card: colonna flessibile per ancorare il link in basso -->
               <div style="padding: 14px 12px 12px 12px; display: flex; flex-direction: column; height: auto;">
                    ...
                    <!-- Spacer per spingere il link al fondo -->
